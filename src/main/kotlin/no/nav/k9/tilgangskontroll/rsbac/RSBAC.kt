package no.nav.k9.tilgangskontroll.rsbac

import no.nav.k9.tilgangskontroll.naudit.Audit
import no.nav.k9.tilgangskontroll.rsbac.DecisionEnums.DENY
import no.nav.k9.tilgangskontroll.rsbac.DecisionEnums.PERMIT
...
open class RSBACImpl<CONTEXT>(private val context: CONTEXT, private val exception: Function<String, RuntimeException> = { RSBACException(it) }) :
    RSBAC<CONTEXT> {
...
class RSBACInstanceImpl<CONTEXT, OUTPUT>(val context: CONTEXT, var exception: Function<String, RuntimeException>) :
    RSBACInstance<CONTEXT> {
    ...
    override fun permit(message: String, rule: Function<CONTEXT, Boolean>): RSBACInstanceImpl<CONTEXT, OUTPUT> = check(
        Policy(message, rule, PERMIT)
    )
    override fun deny(message: String, rule: Function<CONTEXT, Boolean>): RSBACInstanceImpl<CONTEXT, OUTPUT> = check(
        Policy(message, rule, DENY)
    )
